...
# ---------- NEW: UPLOAD / RUNBAT / RUNPS1 / CREATETASK / DELETETASK ----------
...
def upload_info(message):
    ...
    bot.reply_to(
        message,
        "📤 **Upload Mode**\n\n"
        "Send or drag-and-drop any document or photo to this chat. "
        "It will be saved directly to the PC.",
        parse_mode="Markdown"
    )
...
def handle_incoming_files(message):
    ...
    try:
        if message.document:
            ...
            raw_name = message.document.file_name or f"file_{message.document.file_unique_id}"
            # Sanitize: keep basename, strip invalid Windows chars
...
def run_bat_file(message):
    if not is_authorized(message):
        bot.reply_to(message, "Unauthorized user!")
        return
    try:
        bat_path = message.text.replace('/runbat', '').strip()
        if not bat_path or not os.path.exists(bat_path):
            bot.reply_to(message, "Error: Bat file path not found.\nExample: /runbat C:\\path\\to\\script.bat")
            return
        
        result = subprocess.run(bat_path, shell=True, capture_output=True, text=True, encoding='cp850', errors='ignore')
        output = result.stdout if result.stdout else result.stderr
        if not output:
            output = "Batch file executed successfully with no output."
...
@bot.message_handler(commands=["runps1"])
def run_ps1_file(message):
    if not is_authorized(message):
        bot.reply_to(message, "Unauthorized user!")
        return
    ps1_path = message.text[len("/runps1"):].strip().strip('"')
    if not ps1_path or not os.path.exists(ps1_path):
        bot.reply_to(
            message,
            "Error: PS1 file not found.\nExample: `/runps1 C:\\path\\to\\script.ps1`",
            parse_mode="Markdown"
        )
        return
    try:
        ...
        output = result.stdout or result.stderr or "PowerShell script executed successfully (No output)."
        send_long_text(message.chat.id, output, filename="runps1_output.txt", caption="🔷 PS1 Output")
    except subprocess.TimeoutExpired:
        bot.reply_to(message, "Error: PowerShell execution timed out (120s).")
    except Exception as e:
        bot.reply_to(message, f"Error: {str(e)}")


@bot.message_handler(commands=["createtask"])
def create_scheduled_task(message):
    if not is_authorized(message):
        bot.reply_to(message, "Unauthorized user!")
        return
    ...
    if len(parts) < 3:
        bot.reply_to(
            message,
            "Usage: `/createtask <TaskName> <PathToBat>`\n"
            "Example: `/createtask MyTask C:\\script.bat`\n\n"
            "⏰ Task runs **daily** at the current time.",
            parse_mode="Markdown"
        )
        return

    task_name = parts[1].strip()
    bat_path  = parts[2].strip().strip('"')

    if not os.path.exists(bat_path):
        bot.reply_to(message, f"Error: File not found: {bat_path}")
        return

    # Daily task — runs every day at current time
    now = datetime.now()
    ...
    try:
        cmd = (
            ...
            f'/tr "\\"{bat_path}\\"" '
            f'/sc daily /st "{time_str}" /sd "{date_str}" '
            f'/f'
        )
        result = subprocess.run(
            cmd, shell=True, capture_output=True, text=True, startupinfo=startupinfo
        )
        if result.returncode == 0:
            bot.reply_to(
                message,
                f"✅ Daily task `{task_name}` created!\n"
                f"Will run as **SYSTEM** every day at **{time_str}**.",
                parse_mode="Markdown"
            )
        else:
            bot.reply_to(
...
def delete_scheduled_task(message):
    if not is_authorized(message):
        bot.reply_to(message, "Unauthorized user!")
        return
    task_name = message.text[len("/deletetask"):].strip()
    if not task_name:
        bot.reply_to(message, "Usage: `/deletetask <TaskName>`", parse_mode="Markdown")
        return
    try:
        ...
        if result.returncode == 0:
            bot.reply_to(message, f"🗑️ Task `{task_name}` deleted successfully.", parse_mode="Markdown")
        else:
            bot.reply_to(
                message,
                f"Error: {result.stderr.strip() or result.stdout.strip() or 'Task not found'}"
            )
    except Exception as e:
        bot.reply_to(message, f"Error: {str(e)}")            
...
def send_welcome(message):
    if is_authorized(message):
        help_text = (
            ...
            "**--- FILE TRANSFER ---**\n"
            "• `/upload` - Receive files from Telegram to PC\n"
